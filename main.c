...
void process_stream(int sock) {
    ...
    while (1) {
        ...
        for (int i = 0; i < FRAMES_PER_10S; ++i) {
            ...
            // printf("Received frame %d/%d\n", i + 1, FRAMES_PER_10S); // This can be noisy, optionally enable for debug
        }

        // 10秒数据已满，开始处理
        printf("[Check] Size of 10s single-channel buffer: %zu bytes\n", (size_t)BUFFER_SAMPLES * sizeof(int16_t));
 ...
 */
int receive_one_frame_and_extract(int sock, int16_t* target_buffer, uint8_t* frame_buffer) {
    static int is_first_frame = 1; // Flag to print size only for the first frame
    ...
    if (is_first_frame) {
        printf("[Check] Size of one raw frame received: %zd bytes\n", bytes_received);
        is_first_frame = 0;
    }
