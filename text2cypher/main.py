...
prompt_generate_query = ChatPromptTemplate.from_messages(
    [
        ("system",
         ...
         "---"
         "Instructions:\n"
         "1. **CRITICAL RULE: The `RETURN` clause of the query MUST ALWAYS include the `source_id` of the relevant nodes.**\n"
         "2. Use the provided 'nodes' list to identify the primary entities for the query. Match them using `WHERE n.entity_id IN [...]`.\n"
         "3. Use the 'keywords' to understand the user's intent (e.g., find relationships, get descriptions, count nodes).\n"
         "4. If the query asks for relationships between the provided nodes, find paths between them.\n"
         "5. If the query asks for descriptions or properties, return the `description` or other relevant properties of the nodes, but always include `source_id`.\n"
         "---"
         ...
         "Generated Cypher: MATCH (a)-[r]-(b) WHERE a.entity_id = 'Tidal Force' AND b.entity_id = 'Earth Engine' RETURN a.entity_id, a.source_id, type(r), b.entity_id, b.source_id\n"
         "---"
         "Example 2: User wants to know more about 'Helium Flash'.\n"
         "Nodes: ['Helium Flash']\n"
         "Query: 'Tell me about Helium Flash'\n"
         "Generated Cypher: MATCH (n) WHERE n.entity_id = 'Helium Flash' RETURN n.description, n.source_id"
        ),
        ("human",
