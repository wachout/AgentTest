...
// Helper function to perform bit-reversal permutation on the FFT output
static void bit_reversal_permutation(cmplx* buffer, int k) {
    int n = 1 << k;
    for (int i = 0; i < n; i++) {
...
void generate_mel_filterbank(double** filterbank, int n_mels, int n_fft, int sample_rate, double f_min, double f_max) {
    ...
    for (int i = 0; i < n_mels + 2; ++i) {
        ...
        fft_bins[i] = floor((n_fft + 1) * hz_points[i] / sample_rate);
    }

    for (int i = 0; i < n_mels; ++i) {
        if (hz_points[i+2] == hz_points[i]) continue; // Avoid division by zero
...
int compute_mfcc(mfcc_params* params, const double* audio_buffer, size_t buffer_len, double* mfcc_output) {
    ...
    cmplx* fft_buffer = (cmplx*)malloc(sizeof(cmplx) * n_fft);
    double* power_spec = (double*)malloc(sizeof(double) * (n_fft / 2 + 1));
    double* mel_spec = (double*)malloc(sizeof(double) * n_mels);
    ...
    for (int i = 0; i < num_frames; ++i) {
        ...
        for (int j = 0; j < n_mels; ++j) {
            mel_spec[j] = 0.0;
            for (int bin = 0; bin < n_fft / 2 + 1; ++bin) {
                mel_spec[j] += power_spec[bin] * mel_filterbank[j][bin];
            }
        }
